...
if __name__ == '__main__':
    ...
    x_pop = np.linspace(max(psi), solps_psi, n_pop)
    for obj in x_pop:
        psi = np.append(psi, obj)
    print(np.where(ne == ne.min()))
    print(np.where(te == te.min()))
    ad = np.full(n_pop, 10**(-3))
    for k in ad:
        ne = np.append(ne, k)
    df = np.full(n_pop + 4, 10**(-2))
    nt = np.zeros(56)
    for j in range(56):
        nt[j] = te[j]
    te = np.append(nt, df)
