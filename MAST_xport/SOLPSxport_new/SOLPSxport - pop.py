...
class SOLPSxport:
    ...
    # ----------------------------------------------------------------------------------------
    def populate(self, n_tot = 200):
           
        dsa = self.data['solpsData']['dsa']
        n_old = len(dsa)
        x = np.linspace(1, n_old, n_old)
        x_pop = np.linspace(1, n_old, n_tot)
        dsa_f = interpolate.interp1d(x, dsa, fill_value = 'extrapolate')
        
        psi_solps = self.data['solpsData']['psiSOLPS']
        psi_f = interpolate.interp1d(x, psi_solps, fill_value = 'extrapolate')
        
        neold = self.data['solpsData']['last10']['ne']
        neold_f = interpolate.interp1d(x, neold, fill_value = 'extrapolate')
        
        dold = self.data['solpsData']['last10']['dn']
        dold_f = interpolate.interp1d(x, dold, fill_value = 'extrapolate')
        
        teold = self.data['solpsData']['last10']['te']
        teold_f = interpolate.interp1d(x, teold, fill_value = 'extrapolate')
        
        keold = self.data['solpsData']['last10']['ke']
        keold_f = interpolate.interp1d(x, keold, fill_value = 'extrapolate')
        
        tiold = self.data['solpsData']['last10']['ti']
        tiold_f = interpolate.interp1d(x, tiold, fill_value = 'extrapolate')
        
        kiold = self.data['solpsData']['last10']['ki']
        kiold_f = interpolate.interp1d(x, kiold, fill_value = 'extrapolate')
        
        fluxTot = self.data['solpsData']['profiles']['fluxTot']
        fluxTot_f = interpolate.interp1d(x, fluxTot, fill_value = 'extrapolate')
        
        fluxD = self.data['solpsData']['profiles']['fluxD']
        fluxD_f = interpolate.interp1d(x, fluxD, fill_value = 'extrapolate')
        
        fluxConv = self.data['solpsData']['profiles']['fluxConv']
        fluxConv_f = interpolate.interp1d(x, fluxConv, fill_value = 'extrapolate')
        
        qe = self.data['solpsData']['profiles']['qe']
        qe_f = interpolate.interp1d(x, qe, fill_value = 'extrapolate')
        
        qi = self.data['solpsData']['profiles']['qi']
        qi_f = interpolate.interp1d(x, qi, fill_value = 'extrapolate')
        
        self.data['solpsData']['dsa'] = dsa_f(x_pop)
        self.data['solpsData']['psiSOLPS'] = psi_f(x_pop)
        self.data['solpsData']['last10']['dn'] =dold_f(x_pop) 
        self.data['solpsData']['last10']['ne'] = neold_f(x_pop)
        self.data['solpsData']['last10']['te'] = teold_f(x_pop)
        self.data['solpsData']['last10']['ke'] = keold_f(x_pop)
        self.data['solpsData']['last10']['ti'] = tiold_f(x_pop)
        self.data['solpsData']['last10']['ki'] = kiold_f(x_pop)
        
        self.data['solpsData']['profiles']['fluxTot'] = fluxTot_f(x_pop)
        self.data['solpsData']['profiles']['fluxD'] = fluxD_f(x_pop)
        self.data['solpsData']['profiles']['fluxConv'] = fluxConv_f(x_pop)
        self.data['solpsData']['profiles']['qe'] = qe_f(x_pop)
        self.data['solpsData']['profiles']['qi'] = qi_f(x_pop)
    ...
    def calcXportCoef(self, plotit = True, Dn_min = 0.001, chie_min = 0.01, chii_min = 0.01,
        ...
        # pop_dic = self.populate(n_tot = 200)
        
        # dsa = pop_dic['dsa'] 
        # psi_solps = pop_dic['psi_solps']
        # neold = pop_dic['ne']
        # dold = pop_dic['dn']
        # teold = pop_dic['te']
        # keold = pop_dic['ke']
        ...
        # fluxTot = pop_dic['fluxTot']
        # fluxD = pop_dic['fluxD']
        # fluxConv = pop_dic['fluxConv']
        # qe = pop_dic['qe']
        # qi = pop_dic['qi']
